...
from dataset import get_SBEM2_z50_loader

train_loader = get_SBEM2_z50_loader(
    "train",
    args.data_root,
    shuffle=True,
...
test_loader = get_SBEM2_z50_loader(
    "test",
    args.val_data_root,
    shuffle=False,
    num_workers=args.num_workers,
...
ffl = FFL(loss_weight=100, alpha=1.0)  # initialize nn.Module class
l1 = torch.nn.L1Loss()

def total_variation_loss_3d(image_sequence: torch.Tensor) -> torch.Tensor:
    tv_h = torch.sum(torch.abs(image_sequence[:, :, 1:, :] - image_sequence[:, :, :-1, :]))
    tv_w = torch.sum(torch.abs(image_sequence[:, :, :, 1:] - image_sequence[:, :, :, :-1]))
    tv_d = torch.sum(torch.abs(image_sequence[:, 1:, :, :] - image_sequence[:, :-1, :, :]))
    return tv_h + tv_w + tv_d * 0.2
...
def train(args, epoch):
    ...
    for bidx, data_dict in enumerate(train_loader):
        ...
        # Build input batch
        hat = data_dict["stack_hat"]
        tilde = data_dict["stack_tilde"]
        hat_e = data_dict["stack_hat_e"]
        tilde_e = data_dict["stack_tilde_e"]
    
        hat = hat.cuda()
        tilde = tilde.cuda()
        hat_e = hat_e.cuda()
        tilde_e = tilde_e.cuda()
        ...
        hat, tilde, out1, out2 = hat.squeeze(0), tilde.squeeze(0), out1.squeeze(0), out2.squeeze(0)

        l1_loss = l1(out1, tilde) + l1(out2, hat) + 0.1 * l1(out1, out2)
        ssim_loss = 2 - ssim(out1, tilde, data_range=1.0) - ssim(out2, hat, data_range=1.0) + 0.1 * (1 - ssim(out1, out2, data_range=1.0))
        # tv_loss = total_variation_loss_3d(out1) + total_variation_loss_3d(out2)

        loss = l1_loss + ssim_loss #+ tv_loss * 1e-5
