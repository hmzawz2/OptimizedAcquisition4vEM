...
class Encoder2(nn.Module):
    def __init__(self, n_features, kernel_size=3, reduction=4, bias=False, scale_unetfeats=48):
        ...
        self.down01 = nn.Sequential(nn.Conv2d(n_feat0, n_feat, 2, 2, 0, bias=False), nn.LeakyReLU())
        # self.act = act
...
class GShiftNet(nn.Module):
    ...
    def __init__(self, n_features=48, future_frames=0, past_frames=0, in_chl=1):
        ...
        self.feat_extract = nn.Sequential(
            nn.Conv2d(in_chl, self.n_feats0, 3, 1, 1),
            CAB(self.n_feats0, 3, 4, bias=False, act=nn.LeakyReLU()),
            )
        self.conv_last = conv(self.n_feats0, in_chl, 5, bias=False) 
        self.conv_trans = conv(self.n_feats0, self.n_feats0, 3, bias=True)
        self.lrelu = nn.LeakyReLU()
        self.stage1 = Encoder2(self.n_feats2)
        self.orb1 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.orb2 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.orb3 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        # self.orb4 = 
        self.orb4 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.orb5 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.rorb1 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.rorb2 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.rorb3 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.rorb4 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
        self.rorb5 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.LeakyReLU(), bias=False, scale_unetfeats=0)
    ...
    def forward(self, x, k1=None, k2=None, k3=None):
        ...
        out = out.unsqueeze(0)
        out = rearrange(out, 'n d c h w -> n c d h w')
        return out

if __name__ == '__main__':
    net = GShiftNet(future_frames=1).cpu()
    data = torch.randn((1, 1, 5, 64, 64)).cpu()
